...
`openmtgdata classify-schemas --deep-report <path>` verifies and analyzes persisted M3.1 evidence without opening raw archives. It reports structural schema drift and creates a deterministic registry of source-kind-specific `source_interpretation_contract_id`s. Those contracts define structural support only; they do not map columns to semantic fields. The registry preserves unknown additions and blocks unseen fingerprints, reordered or removed required fields, and rows whose width differs from their registered header. An optional `--output` writes a new registry under `intermediate_root` without overwriting an existing file.
