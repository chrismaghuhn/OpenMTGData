...
M5.1 adds evidence-gated Replay normalization through `ReplayAdapterV1`. For the reviewed AFR PremierDraft fingerprint, an accepted raw row is treated as a game-level summary with indexed per-side turn-summary slots; each emitted `ReplayEventV1` is one such slot, not an individual action. Exact source strings and column-index lineage are preserved, while turn count, source `on_play`, slot side, and slot index use explicit deterministic rules. The current registry supports only the AFR PremierDraft Replay schema group; the other 40 Replay groups remain `needs_review`. M5.1 does not join Game data, identify decisions, reconstruct authoritative game state, assert action legality/optimality, create `DecisionSampleV1`, build Parquet, or train models. See [ADR 0002](docs/adr/0002-replay-normalization-v1.md) for the evidence boundary and full-stream limitations.

M5.2 adds `GameRecordV1` source-fact normalization for independently reviewed Game schemas. The current M5.2 registry supports only the AFR PremierDraft Game schema; the other 35 Game groups remain `needs_review`. Mapped fields preserve exact source strings with index-and-name lineage. Source-native identifiers are candidates only and are not approved join keys; source-relative player/outcome fields are not converted to SELF/OPPONENT or policy inputs. Replay/Game joins belong to M6 and actor-relative policy views to M7. No `DecisionSampleV1`, legal/optimal labels, Parquet release, or training is implemented. See [ADR 0003](docs/adr/0003-game-normalization-v1.md).
