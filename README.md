...
OpenMTGData is early infrastructure for building reproducible, provenance-preserving, model-independent Magic: The Gathering datasets. The first intended source is the intentionally published 17Lands Public Datasets. The package can inventory candidate `.csv.gz` filesystem entries, classify canonical basenames, register exact compressed-byte identities with streaming SHA-256, build a versioned source catalog with a semantic source-set digest, validate gzip integrity, and inspect the first CSV record of every registered source to derive strict physical raw-schema fingerprints and schema groups. Header inspection is bounded and does not interpret following records. It does not scan full CSV bodies, infer row types/nullability, interpret columns semantically, join Replay/Game data, extract decisions, build Parquet datasets, publish a dataset, or train a model.
