...
For a candidate decision at slot T, every indexed slot N>T is future information and is forbidden as policy input. Fields in slot T are also unsafe: the source provides no intra-slot ordering proving a state field precedes an action candidate. The report's validator rejects future/current slot selectors and any field without `PRE_TURN`/`TURN_START` timing plus explicit safe policy status. No such safe field currently exists.
