...
All three candidates are classified `ambiguous`; none is approved. The observed equality of numeric game values does not resolve the many-to-many and unmatched row multiplicity or establish source semantic correspondence. There is therefore no supported scoped Replay/Game row join key for this archive pair in M6.1. M6.2 must not emit joined records from these candidates without additional source evidence and a new scoped review.
