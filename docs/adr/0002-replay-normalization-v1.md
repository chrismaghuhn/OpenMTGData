...
**Status:** Accepted for the corrected M5.1 evidence chain, M3.1/M3.2 refreshed on 2026-09-24

**Decision date:** 2026-09-24

**Evidence inputs:** corrected M3.1 deep report `c6c7d6e81c96179f41f32c27e5bfaf52e241a78f00478c8567c12a09ede9ba29`; corrected M3.2 registry `66d58e20fa2adbcbfdcfd927c1074b5807d43af8af94103b61b26fc06f13661c`; semantic source catalog `dcfbae5b65529ea42d637d2337418401f129ab1169db3c9bf0a7d84809573602`.

## Context and observed structure

The registered AFR `.csv.gz` source is gzip containing one TAR member named `replay-data.AFR.PremierDraft.csv`. The corrected source reader selects that member before CSV parsing. The configured corpus now has 40 Replay physical-schema groups representing 100 archives. Every inspected group has a large row-wide header and repeated `user_turn_<n>_*` / `oppo_turn_<n>_*` field families over slots 1 through 30. These are physical header facts. The repeated names alone do not establish event semantics.

M5.1 reviewed the corrected `replay_data_public.AFR.PremierDraft.csv.gz` raw schema (`b4ad0cb27197cf3e37e59d6609a6098da0e0e2f6c4f98b82c45f85fcb7bbea75`) against its refreshed M3.2 structural interpretation contract and processed the complete selected CSV member through M4 reader v2. Its scalar source fields include `turns`, `on_play`, and `game_index`; its per-side indexed families have 30 contiguous slots with the same 33 exact suffixes at each slot. Actual accepted rows contain one total-turn value and per-side slot data. For the reviewed full stream, every normalized row's emitted slot count reconciled with its parsed source `turns` value, and every active slot had at least one populated mapped source field. `on_play` used exact `0`/`1` values for this mapping.
...
For the supported AFR raw-schema group, the final mapping ID is `openmtgdata.replay-field-mapping.v1:a42f925293705fb89936e3b3b5a6d243027d63953808ccfb154920c0a6031140`; the refreshed mapping-registry digest is `43be7080894b55c60f58d569ed8cfeb52a9dbae1fa8688160d91196044ab47f3`. The group contains the PremierDraft and TradDraft archives with the same corrected exact physical header; full-stream review was performed on the PremierDraft representative. The adapter maps exactly 1,980 turn-slot columns using `(column_index, exact_header_name)` selectors, plus the `turns` and `on_play` source columns. Their parser-returned strings, including the exact `turns` and `on_play` lexemes, empty strings, and whitespace, remain unchanged in each event. It adds four deterministic normalized fields: source turn count, source `on_play`, source-side slot family, and source slot index. The turn count parser accepts canonical nonnegative ASCII decimal strings; `on_play` accepts only `0` or `1`. Thus the mapping has 1,982 mapped physical columns (including the two context selectors), preserves both context lexemes, has four deterministic normalization outputs, and emits no reconstructed gameplay fields. The lineage for `source_turn_side` identifies the exact `on_play` column plus `event_ordinal_within_source_record`; `source_turn_slot_index` identifies the event ordinal as its derived input and has no raw value selector.
...
All 40 corrected Replay schema groups are present in the M5.1 mapping registry. One group is `supported`: the AFR raw fingerprint. The other 39 groups remain `needs_review`, even where their headers show similar indexed families. Their exact schemas and refreshed M3 evidence are reported, but this task did not complete M4 full-stream occupancy/normalization review for those fingerprints, so no mapping is applied to them. This is intentional; M3.2 singleton interpretation contracts are not merged by M5.1.
...
The M4 reader v2 completed the selected TAR member, TAR trailer, gzip stream, and registered compressed SHA-256/size checks. The corrected full pass observed 248,072 CSV data records, with 223,404 exact-width records submitted to M5.1 and 24,668 width-mismatched records rejected before normalization. This remains substantially different from bounded M3 prefix evidence and shows why prefix evidence is not full-history coverage. Potential semantic bias from M4 width exclusions remains unresolved; the accepted subset is not asserted to be representative.

M5.1 normalized 223,271 accepted raw records, rejected 133 for invalid/non-supported `turns` values (125) or values exceeding the physical turn-slot bound (8), and emitted 3,977,270 turn-summary events. No partial active slots, zero-event rows, or turn-count mismatches were observed. Normalized records emitted 2–60 events. Event-side counts were 1,990,219 `user` slots and 1,987,051 `oppo` slots. The corrected incremental semantic validation digest is `24db9aba56d48cbf1403a71f128fa870e065df850b9d93db3efdeeac67dbfe08`. Exact counts, mapping IDs, and the turn-count distribution are in ignored local analysis/registry artifacts under `data/intermediate/m5.2/`.
